...
class Agent(object):
    def __init__(self, computation_graph_args, sample_trajectory_args, estimate_advantage_args):
        ...
        self.summary_dir = computation_graph_args['summary_dir']
    ...
    def init_tf_sess(self):
        ...
        self.tb_writer = tf.summary.FileWriter(self.summary_dir, self.sess.graph)
    ...
    def sample_action(self, policy_parameters, min_value=None, max_value=None):
        ...
        if self.discrete:
            ...
            # sy_sampled_ac = tf.constant(0.7)
            
        """             if min_value is not None and max_value is not None:
                        # scale to [0, 1]
                        sy_sampled_ac = (sy_sampled_ac - min_value) / (max_value - min_value)
    ...
    def build_computation_graph(self):
        ...
        if not self.discrete:
            ...
            tf.summary.scalar('sy_mean', tf.reduce_mean(sy_mean)) # temp for debug purposes
            tf.summary.scalar('sy_logstd', tf.reduce_mean(sy_logstd)) # temp for debug purposes
    ...
    def sample_trajectories(self, itr, env):
        ...
        while True:
            ...
            path = self.sample_trajectory(env, animate_this_episode, itr)
            paths.append(path)

            _current_reward = np.sum(path["reward"])
            if _current_reward > _max_reward:
                print(f"================================================= Max reward reached: {_current_reward:.2f} =====================")
                _max_reward = _current_reward

            timesteps_this_batch += pathlength(path)
            if timesteps_this_batch > self.min_timesteps_per_batch:
                self.tb_writer.add_summary(self.summary, itr)
                break
        return paths, timesteps_this_batch

    def sample_trajectory(self, env, animate_this_episode, itr):
        ob = env.reset()
        ...
        while True:
            ...
            ac, self.summary = self.sess.run([self.sy_sampled_ac, self.merged_summary], feed_dict={self.sy_ob_no: np.expand_dims(ob, axis=0)})
            # ac = ac[0]

            # scale action 
            clipped_action = ac
            if isinstance(env.action_space, gym.spaces.Box):
                # scale to [env.action_space.low; env.action_space.high]
                clipped_action = clipped_action * ( env.action_space.high - env.action_space.low) + env.action_space.low
                # Clip the actions to avoid out of bound error
                clipped_action = np.clip(clipped_action, env.action_space.low, env.action_space.high)

            # ac = np.array(ac).squeeze()
            #ac = ac[0]
            acs.append(clipped_action[0])
            ob, rew, done, info = env.step(clipped_action)

            if not done:
                _total_reward += rew
                #print(f'Step with action {clipped_action[0][0]:.3f} was successful, reward is {rew:.2f}, info is: {info}')
                print(f'Step with action {clipped_action} was successful, reward is {rew:.2f}, info is: {info}')
            else:
                #print(f'Step with action {clipped_action[0][0]:.3f} was NOT successful')
                print(f'Step with action {clipped_action} was NOT successful')
                print(f'Episode finished, total reward if {_total_reward:.2f}\n')
...
def train_AC(
    ...
    max_path_length = max_path_length or 100 # env.spec.max_episode_steps
    ...
    for itr in range(n_iter):
        ...
        """         # write events     
                ...
                train_writer = tf.summary.FileWriter(summary_dir, agent.sess.graph)
                tf.global_variables_initializer().run()

                summary = agent.sess.run([merged])
...
def main():
    ...
    parser.add_argument('--env_name', type=str, default='env')
    parser.add_argument('--data', type=str, default='data/')
    parser.add_argument('--exp_name', type=str, default='vac')
    parser.add_argument('--render', action='store_true')
    parser.add_argument('--discount', type=float, default=0.99)
    parser.add_argument('--n_iter', '-n', type=int, default=1000)
    parser.add_argument('--batch_size', '-b', type=int, default=1000)   #  1000
    ...
    summary_path = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'summary')
    if not (os.path.exists(data_path)):
        os.makedirs(summary_path)
    summary_dir = 'sum_' + args.exp_name + '_' + args.env_name + '_' + time.strftime("%d-%m-%Y_%H-%M-%S")
    summary_dir = os.path.join(summary_path, summary_dir)
    ...
    for e in range(args.n_experiments):
        ...
        def train_func():
            train_AC(
                ...
                size=args.size,
                summary_dir=os.path.join(summary_dir,'%d'%seed)
                )
